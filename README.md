...
OSRM не поднимается автоматически вместе с `yarn dev`, его нужно запускать отдельной командой выше.

При первом запуске контейнер сам скачает OSM-данные и подготовит граф маршрутизации (может занять заметное время). После запуска сервис будет доступен по адресу `http://localhost:5563`, проверка готовности: `curl http://localhost:5563/health`.

Не забудьте остановить контейнер, когда он больше не нужен:
